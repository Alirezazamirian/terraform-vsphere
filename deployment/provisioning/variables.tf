...
variable "node_count" {
  description = "each cluster provisioning count"
  type = number
}

variable "networks" {
  description = "Network adaptors with its IPs and details"
  type = list(object({
    name              = string
    ipv4s              = list(string)
...
variable "network_id_map" {
  description = "All available network adaptors to match specifically for each VM's based on template"
  type = any
}
...
variable "memory" {
  description = "VM's memory space (MB)"
  type        = number
...
variable "disks" {
  description = "Required disks to attach the new VM"
  type = list(object({
    label           = string
    size            = number # in GB
    eagerly_scrub   = bool
  ...
  validation {
    condition = alltrue([
      for d in var.disks :
      d.size == null || d.size >= 50
    ])
    error_message = "Each disk must have size >= 50 GB if set."
  }
}

variable "dns_server_list" {
  description = "DNS server list to put at file /etc/resolve.conf"
  type = list(string)
}
